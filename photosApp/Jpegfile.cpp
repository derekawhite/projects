									 ...
									 UINT uiScale)		// bytes!!!
{
	...
	UINT uiWidthBytes;
	uiWidthBytes = WIDTHBYTES(widthPix/uiScale * 24);
	...
	for (uiCount=0;uiCount < height;uiCount++) {
		...
		lOutOff=uiWidthBytes * (uiCount/uiScale);

		bpInAdd= dataBuf + lInOff;
		bpOutAdd= pNew + lOutOff;

		if ( uiCount % uiScale == 0 )
		{
			for ( unsigned int i = 0; i < uiInWidthBytes ; i+=uiScale*3 )
				memcpy(&bpOutAdd[i/uiScale],&bpInAdd[i],3);
		}
	}

	*uiOutWidthBytes=uiWidthBytes/uiScale;
	return pNew;
}
